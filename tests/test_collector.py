...
""")
        ...
        try:
            ...
            assert len(devicea_execs) == 2, f"DeviceA should have 2 executions, got {len(devicea_execs)}"
            
            # Verify DeviceB executions (this is the bug - it would be 0)
            deviceb_execs = [e for e in execution_log if e['device'] == 'DeviceB']
            assert len(deviceb_execs) == 2, f"DeviceB should have 2 executions, got {len(deviceb_execs)}"
