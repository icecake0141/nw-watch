...
Command Outputs コンテナの上部に配置され、Device 列より一段上で、設定されたすべてのデバイスのリアルタイム ping 監視と60秒間の接続状態を表示します。
...
Command Outputs の Side-by-Side Command Output コンテナ上部に Snapshot 操作があります。

**機能**:
- **Capture Snapshot**: 現在表示中の最新コマンド出力を固定表示用に保存
- **Latest / Snapshot**: Side-by-Side Command Output の表示対象を最新データまたは保存済み Snapshot に切り替え
- Snapshot は Diff View ではなく、Command Output コンテナ内の比較対象表示に反映されます
