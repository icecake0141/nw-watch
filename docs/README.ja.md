...
nw-watchに貢献したり、ソースコードを変更したりする場合は、次の手順に従ってください:

#### 1. 開発依存関係を含めてインストール

パッケージを編集可能モードで開発依存関係とともにインストールします:
...
これにより以下がインストールされます:
- 編集可能モード（`-e` フラグ）でのパッケージ。ソースコードへの変更が即座に反映されます
- 開発依存関係（`[dev]` extras）: pytest、pytest-asyncio、httpxなど
