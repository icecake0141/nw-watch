...
def generate_diff(old_text: str, new_text: str) -> str:
    ...
    diff = difflib.unified_diff(
        old_lines, new_lines, fromfile="previous", tofile="latest", lineterm="\n"
    )
...
def generate_inline_char_diff(text_a: str, text_b: str) -> Tuple[str, str]:
    ...
    for tag, i1, i2, j1, j2 in matcher.get_opcodes():
        ...
        if tag == "equal":
            ...
            # Text was replaced
            result_a.append(
                f'<span class="char-diff-remove">{html.escape(text_a_part)}</span>'
            )
            result_b.append(
                f'<span class="char-diff-add">{html.escape(text_b_part)}</span>'
            )
        elif tag == "delete":
            # Text only in A
            result_a.append(
