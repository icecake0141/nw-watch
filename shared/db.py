...
class Database:
    ...
    def _init_schema(self):
        ...
        # Create indexes
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_runs_device_command ON runs(device_id, command_id)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_runs_ts ON runs(ts_epoch)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_ping_device_ts ON ping_samples(device_id, ts_epoch)')
    ...
    def insert_run(self, device_name: str, command_text: str, ts_epoch: int,
                   output_text: str, ok: bool, error_message: Optional[str] = None,
                   duration_ms: Optional[float] = None, is_filtered: bool = False,
                   is_truncated: bool = False, original_line_count: int = 0):
        ...
        cursor.execute('''
            INSERT INTO runs (device_id, command_id, ts_epoch, output_text, ok, 
                            ...
                            original_line_count)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (device_id, command_id, ts_epoch, output_text, 1 if ok else 0,
              error_message, duration_ms, 1 if is_filtered else 0,
              1 if is_truncated else 0, original_line_count))
        self.conn.commit()
        
        # Clean up old runs, keep only max_runs
        self._cleanup_old_runs(device_id, command_id, max_runs=10)
    
    def _cleanup_old_runs(self, device_id: int, command_id: int, max_runs: int = 10):
        """Remove old runs, keeping only the most recent max_runs."""
        cursor = self.conn.cursor()
        cursor.execute('''
            DELETE FROM runs
        ...
        ''', (device_id, command_id, device_id, command_id, max_runs))
        self.conn.commit()
    
    def insert_ping_sample(self, device_name: str, ts_epoch: int, ok: bool,
    ...
    def get_latest_runs(self, device_name: str, command_text: str, 
        ...
        cursor.execute('''
            ...
            LIMIT ?
        ''', (device_name, command_text, limit))
        
        rows = cursor.fetchall()
        return [dict(row) for row in rows]
    
    def get_latest_run(self, device_name: str, command_text: str) -> Optional[Dict[str, Any]]:
        """Get the most recent run for a device/command."""
        runs = self.get_latest_runs(device_name, command_text, limit=1)
